...
import RoomItem, { RoomItemProps } from "@/components/rooms/RoomItem";

const mails: RoomItemProps[] = [
  {
...
export default function HomePage() {
  ...
  return (
    <div className="h-full w-full flex max-w-screen-2xl p-8 mx-auto">
      <div className="px-6 flex-1 my-3">
        ...
        <div className="max-w-screen-2xl">
          <div className="relative w-auto flex pb-2">
            <Search
              size="sm"
            ...
            <Input
              ...
              className="pl-8 w-auto max-w-md flex-1"
            />
          </div>

          <ScrollArea className="h-screen my-4">
            <div className="grid md:grid-cols-2 grid-cols-1 flex-wrap gap-4">
              {isLoading && mails.map(() => <RoomItemSkeleton />)}
              {!isLoading && mails.map((item) => <RoomItem {...item} />)}
            </div>
          </ScrollArea>
        </div>
      </div>
